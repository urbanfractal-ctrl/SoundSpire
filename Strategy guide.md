# TTRPG Companion App — Strategic Development Summary v4
...
- **Mode A (Autonomous):** app drives mood transitions automatically — assigned to expressive GMs whose vocal delivery varies measurably across mood passages during enrollment
- **Mode B (Collaborative):** app suggests moods, GM confirms with a keypress — assigned to measured/monotone GMs and any GM who prefers direct control. Mood palette visible during session. Optional numpad mood mappings for dedicated hardware.
- Vocal delivery threshold detection during enrollment routes each GM into the appropriate mode automatically — no manual setup
...
1. **Voice enrollment:** “Let’s get to know your voice. Read these passages aloud.” Seven mood-specific passages, each approximately 60 seconds. The app captures the GM’s vocal fingerprint and mood-specific delivery characteristics simultaneously.
1. During training, the app plays sample music matching each mood — the GM *experiences* the product while teaching it their voice. The setup IS the demo.
1. “Great. I’ll be listening for you during your sessions. Let’s set up your music.”
1. BYOM import (or proceed with bundled library)
...
### Delivery Threshold Detection — Mode Routing

After all seven passages are recorded, the app performs a final enrollment step that the GM doesn’t see: it computes mean pairwise cosine distance across the seven emotion2vec embeddings. This measures how much the GM’s vocal delivery actually varies across emotional contexts. The result routes each GM into one of two operating modes.

**Mode A (Autonomous):** The GM’s delivery varies measurably across the seven moods — they unconsciously shift pitch, pace, and energy when narrating tense scenes versus calm ones. The app can ride this signal. Mood transitions fire automatically. The GM corrects mistakes with hotkeys, and those corrections feed back into the learning loop.

**Mode B (Collaborative):** The GM’s delivery is measured and consistent — a lot of experienced GMs narrate this way. Controlled, professional, character-voice-heavy delivery doesn’t produce the variation the classifier needs to work confidently. Rather than misfire constantly, the app shifts into a confirmation model: it detects keywords and surfaces mood suggestions, the GM confirms with a single keypress. The GM is the director; the app is the attentive stagehand. This is not a degraded experience — it’s a different design contract, and the framing matters enormously.

The post-enrollment screen delivers the routing result honestly:

- **If Autonomous:** “You’re all set. The app will handle mood shifts automatically. Use the hotkeys to correct it when it gets things wrong — it learns from every correction.”
- **If Collaborative:** “Your style, your control. Your narration is measured and consistent — a lot of experienced GMs narrate this way. The app will suggest moods as you play; you confirm with a single keypress. You’re the director.”

Both screens lead to BYOM setup. The GM can change their mode preference in pre-session settings at any time — this is a preference, not a permanent classification.

**Why this matters strategically:** It solves the product’s biggest single failure mode. Without Mode B, a significant fraction of GMs would experience the system misfiring constantly and conclude that AI detection doesn’t work. With Mode B, those GMs get a different promise — one the product can reliably keep. The word-of-mouth story from Mode B GMs is “the app always gets it right, I just confirm” rather than “the AI is kind of a mess.”
...
1. **Shift** (`Ctrl+↑/↓` or mood-cycle key) — Different mood entirely. The app misread the scene. GM taps the correct mood. Transition uses a designed crossfade: faster fade-down, brief “musical breath” dip, new mood fades in.
1. **Hold / Lock** (`Ctrl+Space`) — Stay put, stop making decisions. The app is being too active in a moment where the GM wants atmospheric neutrality. One press freezes current playback and suspends detection. Another press releases.

### Mode A (Autonomous) — Live Session

This is the default experience for GMs with expressive vocal delivery. The app behaves as described throughout this document: it listens, decides, and transitions. The GM’s only interaction during play is correction via hotkeys when the system gets it wrong. The three-hotkey interface covers everything.

Visible footprint: system tray icon showing current mood color. A small popover on click. Nothing else. The app is essentially invisible.

### Mode B (Collaborative) — Live Session

Mode B adds one persistent UI element during sessions: a mood palette. A narrow horizontal strip — docked to the edge of the screen, always-on-top but non-intrusive — displaying all seven mood categories. When the keyword engine detects a likely mood shift, it highlights the suggested category on the palette. The GM sees the suggestion and confirms with a click, a numpad key, or ignores it.

This is not a worse experience. It’s a different promise: the GM is always in control, the app is never wrong without their agreement, and the experience never breaks down. For methodical or low-variance GMs, this is the better fit.

**Numpad integration:** Mode B GMs who own a numpad or macropad can assign each of the seven moods to a dedicated key. With the physical layout mapping spatial logic (Calm at 5, Combat at 6, Tension at 9, etc.), a practiced GM can confirm mood changes without looking away from the table. This turns hardware that most GMs own and never think about into a dedicated session controller.

The three correction hotkeys (next, shift, hold/lock) remain active in Mode B — they apply to whatever mood is currently playing, same as in Mode A.
...
1. **Powers BYOM auto-suggestion** — for Pro users, the system pre-sorts imported music into mood buckets; the GM reviews for obvious mistakes (fast) rather than manually tagging everything (slow)
1. **Informs the crossfade engine** — key, tempo, and spectral profiles enable smarter transitions between tracks (same-key transitions, compatible tempo matching)

### BYOM Onboarding Flow

1. GM imports tracks from device storage (drag-and-drop from file explorer on desktop)
1. App runs Layer 1 auto-analysis, pre-sorts into seven mood buckets
1. GM reviews the sorting — corrects obvious errors, approves the rest
1. Free tier: broad categories only. Pro tier: subcategory suggestions available.
1. Genre tagging happens naturally during campaign selection
...
1. Frame as situations: “Combat is about to start. What words or phrases does the GM typically say at this moment?” — captures official terms, slang, shorthand, and English loanwords simultaneously
1. Include open-ended questions for language-specific triggers not anticipated
1. Deploy to target language TTRPG communities
1. Curate via 1–2 trusted community volunteers per language
1. Build vocabulary file from curated responses
...
1. App runs automatic broad mood analysis (Layer 1) — pre-sorts into seven categories
1. GM reviews the auto-sort, corrects obvious errors, approves the rest
1. Free tier: broad categories only. Pro tier: subcategory auto-suggestions available for review.
1. Pre-session only — complexity lives in prep, not play.
...
1. **All processing local.** Audio never leaves the device. No cloud transmission of voice data. “Your voice never leaves your device, ever.” This is both a GDPR simplification and a marketing asset.
1. **No raw audio storage.** Audio processed transiently through Whisper and emotion2vec, converted to abstract numerical features (text transcription, embeddings), then immediately discarded. Embeddings cannot be reversed into voice recordings. Document this in the DPIA.
1. **Portable, deletable vocal profile package.** Speaker verification embedding, emotion2vec mappings, accumulated session corrections — all in one encrypted container per GM. Clear schema, versioned, encrypted at rest.
1. **Right to erasure button.** “Delete my voice profile” wipes everything: speaker embedding, mood mappings, all session correction history. Available from day one. When tablet sync exists (Phase 2), deletion propagates across all synced devices.
1. **DPIA completed before launch.** Documents what data is collected, why, how processed, where stored, what risks exist, and what mitigations are implemented. Can be drafted by the founder; must be reviewed by a lawyer.
...
- **The magic demo problem:** How do you demonstrate real-time mood detection in marketing without a live session? (Voice training as demo partially solves this — the enrollment flow IS the demo.)
- **Name and brand identity:** Nothing decided yet. (OQ-06)
...
- **DELIVERY_THRESHOLD calibration:** The threshold value (0.18 mean pairwise cosine distance) is an untested hypothesis. Must be validated against enrollment data from at least 20 diverse GMs during beta — expressive performers, monotone narrators, NPC-voice-heavy GMs, quiet speakers. Wrong threshold means GMs are misrouted and the mode promise breaks down. This is Critical — must be validated before G1.
- **Mode B palette position:** The mood palette must be visible without demanding attention. Proposed: horizontal strip docked to bottom edge of a narrow persistent window (not tray popover — too many clicks). Exact dimensions, z-order, and always-on-top behavior need a design spike on both Windows and macOS.
- **Numpad hotkey conflicts:** Numpad keys conflict with standard numeric input when a text field is focused. Need a rule: suppress numpad hotkeys when any text input has OS focus, or document as “close text fields during sessions.”
...
1. **Microphone quality testing:** Record a real game session (or simulate with friends) on 2–3 different laptops. Run through whisper.cpp medium. Test keyword detection accuracy in real tabletop chaos. This is the remaining existential risk.
1. **emotion2vec base integration:** Test inference speed. Extract embeddings from sample narration audio. Confirm it runs alongside whisper.cpp without resource contention on target hardware.
1. **Audio engine selection:** FMOD vs native. Build a quick prototype: crossfade between two tracks on a hotkey. Layer a one-shot SFX on top. Does the transition feel seamless?

### Content & Design (Parallel)

1. **Audio analysis tool prototype:** librosa feature extraction → seven-mood rule-based classification. Test against a sample of 50 CC tracks. Measure classification accuracy.
1. **Voice training passage design:** Write seven evocative fantasy narration passages in English and Greek. Must pull genuine vocal performance. Greek passages written natively, not translated.
1. **Fantasy keyword vocabulary:** Draft 15–20 high-confidence trigger phrases per mood category. Test against real GM narration captured through a laptop mic.
1. **Greek keyword vocabulary:** Draft core 30–40 universal fantasy triggers in authentic Greek table language. Test mixed-language detection with medium Whisper model.
1. **Audio licensing audit:** Identify and legally clear 35 music tracks + 25 sound effects. Build content database with per-asset license tracking from day one.
1. **Commission originals:** Contract 10–15 ambient tracks for critical mood categories.

### Compliance (Before Launch)

1. **Draft DPIA:** Document data flows, processing purposes, risks, mitigations.
1. **Draft Article 6(3) exception argument:** Formal document for why the system is non-high-risk.
1. **Design consent flow:** Voice training setup screens with granular consent checkboxes and clear disclosures.
1. **Legal review:** Single consultation with EU AI Act / GDPR specialist. Budget €500–1,500.
...
*Document v4 — Incorporates desktop platform pivot, dual-signal detection architecture, emotion2vec foundation model, continuous learning strategy, CC-licensed content library research, SFX inclusion, audio analysis intelligence layer, EU AI Act / GDPR compliance architecture, vocal delivery threshold detection, and Mode A (Autonomous) / Mode B (Collaborative) session routing. Living document — to be updated as technical spikes are completed and decisions are validated.*
