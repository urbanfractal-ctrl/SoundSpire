...
1. [Tech Stack — Locked Decisions](#3-tech-stack--locked-decisions)
1. [Architecture Overview](#4-architecture-overview)
1. [Audio Processing Pipeline — Detailed](#5-audio-processing-pipeline--detailed)
1. [ML Model Strategy — ONNX-Native](#6-ml-model-strategy--onnx-native)
1. [Non-Negotiable Constraints](#7-non-negotiable-constraints)
1. [Platform-Specific Requirements](#8-platform-specific-requirements)
1. [Startup Sequence & Phase Model](#9-startup-sequence--phase-model)
1. [Build, Run & Test Commands](#10-build-run--test-commands)
1. [CI/CD Pipeline](#11-cicd-pipeline)
1. [Module Breakdown & Acceptance Criteria](#12-module-breakdown--acceptance-criteria)
1. [Data Schemas — Canonical Definitions](#13-data-schemas--canonical-definitions)
1. [Detection FSM — Complete Specification](#14-detection-fsm--complete-specification)
1. [IPC & Event Contracts](#15-ipc--event-contracts)
1. [Coding Conventions](#16-coding-conventions)
1. [Testing Strategy](#17-testing-strategy)
1. [Performance Budgets](#18-performance-budgets)
1. [Phase Gates](#19-phase-gates)
1. [Open Questions Register](#20-open-questions-register)
...
### Spike Exemptions — The Vibe Coding Zone

This document is intentionally strict. That strictness exists to protect the
production codebase from architectural drift across sessions and contributors.
It is not meant to be a barrier to fast exploration.

**Code in `spikes/` is explicitly exempt from all rules in this document.**

When working in `spikes/`, you may:

- Use Python sidecars, scripts, or notebooks instead of ONNX-native Rust
- Ignore schema definitions, migration rules, and data model requirements
- Skip error handling, bounded channels, and performance budgets
- Use unbounded channels, global state, `unwrap()` everywhere, magic numbers
- Prototype with placeholder audio, hardcoded paths, and mock data
- Ignore CI, linting, test coverage, and coding conventions entirely
- Use whatever library, language, or approach proves the concept fastest

**The only obligation when working in `spikes/`:**

Before the spike directory is deleted or archived, write a short `README.md`
inside it documenting:

- What question the spike was trying to answer
- What you actually found out (positive or negative)
- Any numbers that matter (latency, accuracy, memory, etc.)
- The decision that follows from the result

That’s it. No other formalities. The spike exists to answer a question fast.
The README exists so the answer survives after the code is gone.

**What is never exempt, even in spikes:**

- Committing real voice recordings or biometric data to the repo
- Committing API keys, credentials, or personal data of any kind
- Writing spike output directly into `src-tauri/src/` or `src/` directories

Spike code that has proven its concept and is being promoted to production
must be rewritten to comply with this document before merging. No exceptions.
“It worked in the spike” is not a reason to skip the architectural constraints —
it is the reason to implement them correctly.
...
│   └── .gitignore
│
├── spikes/                            ← Throwaway exploration code (see §1 Spike Exemptions)
│   └── .gitkeep                       ← All spike dirs contain README.md before deletion
│                                         Audio files, model checkpoints, and large binaries
│                                         in spikes/ are gitignored via root .gitignore
│
└── tests/
    ├── integration/
    │   ├── pipeline_test.rs
...
**Enrollment flow — threshold step (non-optional):**

After all 7 passages are recorded and embeddings extracted, the enrollment process performs a delivery threshold calculation before writing the profile complete state:

```
STEP: Threshold calculation
- Compute mean pairwise cosine distance across the 7 emotion2vec embeddings
- Compare against DELIVERY_THRESHOLD constant (see detection/threshold.rs)
- Write result to enrollment_result table:
    delivery_variance, threshold_passed, threshold_value, enrolled_at
- Write preferred_mode to app config table:
    threshold_passed = true  → preferred_mode = 'Autonomous'
    threshold_passed = false → preferred_mode = 'Collaborative'

STEP: UX routing reveal
Do NOT proceed silently. Show one of two distinct post-enrollment screens:

  If threshold_passed = true (Autonomous):
    Title: "You're all set."
    Body:  "The app will handle mood shifts automatically during your sessions.
            Use the hotkeys to correct it when it gets things wrong — it learns
            from every correction."
    CTA:   "Set up your music →"

  If threshold_passed = false (Collaborative):
    Title: "Your style, your control."
    Body:  "Your narration is measured and consistent — a lot of experienced GMs
            narrate this way. The app will suggest moods as you play; you confirm
            with a single keypress. You're the director."
    CTA:   "See how it works →" (shows Mode B palette preview before continuing)

Both screens proceed to BYOM import / bundled library setup.
The GM can change their preferred_mode in pre-session settings at any time —
this is a preference, not a permanent classification.
```
...
- [ ] delivery_variance computed and written to profile DB for every enrollment
- [ ] enrollment_result row exists after complete_enrollment() returns
- [ ] preferred_mode written to config table matching threshold result
- [ ] Threshold-passed screen shown when delivery_variance ≥ DELIVERY_THRESHOLD
- [ ] Threshold-failed screen shown when delivery_variance < DELIVERY_THRESHOLD
- [ ] Mode B palette preview renders correctly before BYOM setup
- [ ] preferred_mode overridable in pre-session settings without re-enrollment
...
- **Mode B (Collaborative) session UI:**
  - Narrow persistent palette window: horizontal strip, always-on-top, docked
    to screen edge (exact position TBD — see OQ-12 for design spike requirement)
  - Palette shows all 7 mood categories; highlights suggested mood from keyword
    engine. GM clicks to confirm or ignores.
  - Numpad hotkeys registered when session_mode = Collaborative; unregistered
    on session end or mode switch. Never registered in Autonomous mode.
  - Tray icon behavior identical to Mode A
- Transition type routing per `MoodCategory::transition_to()` rules
- Error display: single status line only, no modal, no popup
...
- [ ] Mode B palette renders for Collaborative GMs; NOT shown for Autonomous GMs
- [ ] Numpad hotkeys registered only when session_mode = Collaborative
- [ ] Numpad hotkeys unregistered on session end (verified: no numpad conflict post-session)
...
INSERT OR IGNORE INTO config (key, value) VALUES
    ...
    -- Session mode preference — set from enrollment threshold result.
    -- Overridable by user at any time. Only 'Autonomous' and 'Collaborative'
    -- are valid values. System states (KeywordOnly, Manual) are never persisted here.
    ('session_mode',                   'Autonomous'),

    -- Numpad mood mappings — Mode B only. Registered only when session_mode = Collaborative.
    -- Default layout: Calm at center (5), escalation up-right, subdued states lower-left.
    -- Set to empty string '' to disable a numpad key (not null).
    ('numpad_mood_calm',               'Numpad5'),
    ('numpad_mood_tension',            'Numpad9'),
    ('numpad_mood_combat',             'Numpad6'),
    ('numpad_mood_mystery',            'Numpad8'),
    ('numpad_mood_travel',             'Numpad4'),
    ('numpad_mood_celebration',        'Numpad2'),
    ('numpad_mood_sorrow',             'Numpad7'),
    ('numpad_hold_lock',               'Numpad0');
...
-- Migration 002: Vocal delivery threshold result
-- File: src-tauri/src/db/profile_migrations/002_delivery_threshold.sql
-- delivery_variance: mean pairwise cosine distance across 7 emotion2vec embeddings.
--   Range 0.0–1.0. Higher = more expressive vocal delivery variation across moods.
-- threshold_passed: 1 if delivery_variance ≥ DELIVERY_THRESHOLD, 0 otherwise.
-- threshold_value: snapshot of DELIVERY_THRESHOLD constant at enrollment time.
-- preferred_mode: 'Autonomous' or 'Collaborative' only — system states never stored here.

ALTER TABLE mood_samples ADD COLUMN
    delivery_variance REAL;     -- null until all 7 passages recorded

CREATE TABLE IF NOT EXISTS enrollment_result (
    id                TEXT PRIMARY KEY DEFAULT 'primary',
    delivery_variance REAL NOT NULL,
    threshold_passed  INTEGER NOT NULL CHECK (threshold_passed IN (0, 1)),
    threshold_value   REAL NOT NULL,  -- snapshot of DELIVERY_THRESHOLD at enrollment time
    enrolled_at       DATETIME NOT NULL
);

INSERT OR IGNORE INTO profile_schema_migrations (version) VALUES ('002');
```

**Threshold constant** (`src-tauri/src/detection/threshold.rs`):

```rust
// Minimum mean pairwise cosine distance across seven mood embeddings
// for a GM to be routed into Autonomous mode.
// Value is a hypothesis — must be validated against real enrollment data
// from ≥ 20 diverse GMs during beta (OQ-11). Adjust with documented evidence only.
pub const DELIVERY_THRESHOLD: f32 = 0.18;
```
...
pub enum DetectionMode {
    #[default]
    Autonomous,    // App drives. Mood transitions fire automatically on keyword match.
                   // GM corrects with Next / Shift / Hold hotkeys.
                   // Assigned to GMs who pass the vocal delivery threshold at enrollment.

    Collaborative, // App suggests. GM confirms. Mood palette visible during session.
                   // Keyword engine highlights suggested mood; no auto-transition.
                   // Assigned to GMs below vocal threshold, or any GM who prefers control.
                   // GM confirms with palette click or configured numpad key.

    KeywordOnly,   // System state — not a user choice.
                   // Speaker verification failed to init, or vocal_analysis_consent declined.
                   // Behaviour: keyword detection active, no speaker filtering.
                   // Transparent to user — no error state shown.

    Manual,        // System state — not a user choice.
                   // Detection thread in failed/unrecoverable state.
                   // Hotkeys remain active. Auto-detection suspended.
                   // User sees a status indicator; not an error dialog.
}

/// Invariant: Autonomous and Collaborative are persisted user preferences.
/// KeywordOnly and Manual are runtime system states — never written to config
/// table as user preferences, only surfaced transiently in SessionState.
...
// Autonomous and Collaborative are user preferences (persisted to config).
// KeywordOnly and Manual are runtime system states (transient, never persisted).
export type DetectionMode = 'Autonomous' | 'Collaborative' | 'KeywordOnly' | 'Manual';
export type StartupPhase = 'initializing' | 'ui_ready' | 'detection_ready' | 'error';
...
export interface EnrollmentCompletePayload {
  gmId: string;
  deliveryVariance: number;       // 0.0–1.0 — mean pairwise cosine distance
  thresholdPassed: boolean;
  assignedMode: 'Autonomous' | 'Collaborative';  // never KeywordOnly or Manual
}
// Note: Frontend routing to post-enrollment screen is based on assignedMode only.
// Do NOT derive routing by inspecting deliveryVariance against a hardcoded threshold —
// the backend owns that logic. Render based on assignedMode from the payload.
```
...
complete_enrollment(gm_id: String) -> Result<EnrollmentSummary, String>
// EnrollmentSummary includes routing decision from threshold calculation:
// pub struct EnrollmentSummary {
//     pub profile: GmProfile,
//     pub delivery_variance: f32,
//     pub threshold_passed: bool,
//     pub assigned_mode: DetectionMode,  // always Autonomous or Collaborative
// }
...
|OQ-11|DELIVERY_THRESHOLD calibration        |0.18 is an untested hypothesis. Must be validated against enrollment data from ≥ 20 diverse GMs during beta — expressive performers, monotone narrators, NPC-voice-heavy GMs, quiet speakers. Wrong threshold means GMs are misrouted and the mode promise breaks down.            |Critical — before G1            |
|OQ-12|Mode B palette position during session|The mood palette must be visible without demanding attention. Proposed: horizontal strip docked to bottom of a narrow persistent window (not tray popover — too many clicks). Exact dimensions, z-order, and always-on-top behaviour need a design spike on both Windows and macOS.|High — M6 dependency            |
|OQ-13|Numpad hotkey conflict detection      |Numpad keys conflict with standard numeric input when a text field is focused. Tauri’s global hotkey API fires regardless of focus. Need a rule: numpad hotkeys suppressed when any text input has OS focus? Or document as “close text fields during sessions”?                   |Medium — decide before M6 ships |

-----

*CLAUDE.md v3 — Synchronized with MVP Strategy v4.*
*Incorporates all of v2.0 plus: vocal delivery threshold detection and scoring,*
*Mode A (Autonomous) / Mode B (Collaborative) routing, DetectionMode enum*
*extension (four values: Autonomous, Collaborative, KeywordOnly, Manual),*
*enrollment_result schema, post-enrollment UX routing screens, session_mode*
*and numpad config keys, EnrollmentCompletePayload IPC event, EnrollmentSummary*
*command return type, profile_migrations/002_delivery_threshold.sql.*
*No locked decisions from v2.0 were altered.*
*Version: 3.0*
